...
- Navigation-focused article headings suppress the browser's default outline (including Safari's blue rectangle) through a temporary `data-reading-nav-target` marker, removed on blur. Heading focus is retained for assistive technology; keyboard focus outlines on interactive links and buttons remain unchanged.
