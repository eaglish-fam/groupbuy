...
(function(root,factory){const api=factory();if(typeof module==='object')module.exports=api;else root.ProductContent=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function mountVideos(container,input){const list=videos(input);if(!list.length){container.textContent='尚未提供影片';return;}const nav=document.createElement('div'),panel=document.createElement('div');nav.className='video-choices';nav.setAttribute('aria-label','選擇影片');panel.className='video-panel';const select=(v,i)=>{panel.replaceChildren();[...nav.children].forEach((b,j)=>b.setAttribute('aria-pressed',String(j===i)));const a=document.createElement('a');a.href=v.url;a.target='_blank';a.rel='noopener noreferrer';a.textContent='若影片無法播放，在原平台觀看 ↗';if(v.embedUrl){const frame=document.createElement('iframe');frame.src=v.embedUrl;frame.title=v.title;frame.loading='lazy';frame.allow='accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share';frame.allowFullscreen=true;frame.referrerPolicy='strict-origin-when-cross-origin';frame.className=v.vertical?'is-vertical':'is-landscape';frame.dataset.provider=v.embedType;panel.append(frame);}else{const p=document.createElement('p');p.textContent='這個平台目前不支援站內播放。';panel.append(p);}panel.append(a);};list.forEach((v,i)=>{const b=document.createElement('button');b.textContent=v.title;b.type='button';b.onclick=()=>select(v,i);nav.append(b);});container.append(nav,panel);select(list[0],0);}
