...
export function scene(id,{priority=false,card=false}={}){
 ...
 return `<img src="/trip/assets/${id}.webp" srcset="/trip/assets/${id}-640.webp 640w, /trip/assets/${id}.webp 1440w" sizes="${card?'(max-width:700px) 90vw, 380px':'(max-width:700px) 90vw, 760px'}" alt="${esc(m.alt)}" width="1440" height="${m.height}" ${priority?'fetchpriority="high"':'loading="lazy"'} decoding="async">`;
}
...
export const film=(second,label='看我們的體驗片段')=>external(bangkokCatalog.video+`&t=${second}s`,label);
...
export function cardsSection(){return `<section class="section wrap" id="places"><p class="eyebrow"><b>01</b> / BANGKOK PLACES</p><h2>曼谷景點，先看這三站</h2><p class="section-intro">市集與兒童館在恰圖恰一帶；水族館在 Siam。照區域安排，每天都有時間好好玩。</p><div class="place-grid">${bangkokCatalog.places.map(placeCard).join('')}</div></section>`;}
export function placeSection(p,index){
 return `<section id="${p.anchor}" data-place-id="${p.id}"><p class="eyebrow"><b>${String(index+1).padStart(2,'0')}</b> / ${p.area.toUpperCase()}</p><h2>${p.name}</h2><p class="place-english">${p.englishName}</p><p>${p.intro}</p><figure>${scene(p.image)}<figcaption>${media[p.image].caption}</figcaption></figure><dl class="bkk-facts"><div><dt>建議停留</dt><dd>${p.duration}</dd></div><div><dt>開放時間</dt><dd>${p.hours}</dd></div><div><dt>怎麼去</dt><dd>${p.transport}</dd></div><div><dt>雨天安排</dt><dd>${p.weather}</dd></div></dl>${p.activities.map(a=>`<h3>${a.title}</h3><p>${a.text}</p>`).join('')}<div class="bkk-gallery ${p.gallery.length===1?'single':''}">${p.gallery.map(id=>`<figure>${scene(id)}<figcaption>${media[id].caption}</figcaption></figure>`).join('')}</div><aside class="bkk-experience"><h3>我們在這裡的小故事</h3><p>${p.experience}</p></aside><p class="bkk-tip"><strong>出發前準備：</strong>${p.tip}</p><div class="actions">${maps(p)}${film(p.videoSeconds)}</div>${p.booking.length?`<aside class="bkk-booking"><h3>想去水族館？先選日期與票種</h3><p>比較相同的入場日、旅客身分與包含項目，再看總價。玻璃底船與 4D 電影不一定包含在基本門票裡。</p><div class="actions">${p.booking.map(b=>external(b.url,b.label,'button',b.affiliate)).join('')}</div><p class="affiliate-note">本文的 Klook／KKday 連結為聯盟連結；透過連結購買，我們可能獲得佣金。價格與方案依預訂頁顯示。</p></aside>`:''}<details class="bkk-sources"><summary>查看營業與參觀資訊來源</summary><p>行前資訊核對：${bangkokCatalog.checkedAt}。臨時休館、活動場次與票券條件，請在出發前再次確認。</p><div class="actions">${p.sources.map(s=>external(s.url,s.label)).join('')}</div></details></section>`;
}
