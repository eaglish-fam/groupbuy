...
const stepsHtml=steps=>`<ol>${steps.map(s=>`<li><time>${esc(s.time)}</time><div>${s.anchor?`<a href="#${s.anchor}">${esc(s.title)}</a>`:s.mapsQuery?`<a href="https://www.google.com/maps/search/?api=1&amp;query=${encodeURIComponent(s.mapsQuery)}" target="_blank" rel="noopener">${esc(s.title)} ↗</a>`:esc(s.title)}<p>${esc(s.text)}</p></div></li>`).join('')}</ol>`;
const routeHtml=r=>`<details class="bkk-day" id="route-${r.id}" data-route-id="${r.id}"><summary>${esc(r.label)}</summary><p class="eyebrow">${esc(r.area)}</p><p class="route-description" data-pace="relaxed">${esc(r.duration)}。${esc(r.condition)}</p><p class="route-description" data-pace="compact" hidden>${esc(r.compactDuration)}。${esc(r.condition)}</p><div class="bkk-pace-schedule" data-pace="relaxed">${stepsHtml(r.steps)}</div><details class="bkk-compact-variant" data-pace="compact"><summary>緊湊版時間表</summary><p class="bkk-compact-note">${esc(r.compactNote)}${r.compactSourceUrl?` <a href="${esc(r.compactSourceUrl)}" target="_blank" rel="noopener">${esc(r.compactSourceLabel)} ↗</a>`:''}</p>${stepsHtml(r.compactSteps)}</details></details>`;
export function routeSection(){
 ...
 <p id="bkk-pace-help" class="bkk-control-help">緊湊版會改變當天時間表；Safari World 仍只排白天，機場區須先看班機時間。</p>
 ...
 <details class="bkk-plan-notes"><summary>排日期前，先看這些提醒</summary><p>這些是按區域重組的建議，<strong>不是影片的 Day 1、Day 2，也不含抵達和離境日</strong>。市集優先排週末，並避開休館日；票券、車程和活動場次需另外確認。蛇園只是一個機場區半日選項，不順路就留作自由活動。</p><p>時間是規劃額度，不是交通或場次保證；試排工具不會自動核對你出發日期的營業狀態。</p><a href="#rain">遇到下雨，怎麼換？</a></details>
 <script type="application/json" id="bkk-planner-data">${JSON.stringify(config).replaceAll('<','\\u003c')}</script><script type="module" src="/trip/bangkok-planner.mjs?v=20260926-2"></script></section>`;
}
