...
export function routeSection(){
 ...
 return `<section id="plan" class="bkk-planner"><p class="eyebrow"><b>PLAN</b> / YOUR BANGKOK DAYS</p><h2>曼谷 2～5 日，選好天數就能試排</h2><p>先選遊玩天數，再點選某一天看路線；想看恐龍或動物，也可以直接換區域。</p>
 ...
 <p class="bkk-planner-fallback">展開你想去的區域，查看一日安排。可以從市集＋水族館兩天開始，再加上動物園或河畔。</p>
 <div class="bkk-route-panels">${bangkokRoutes.routes.map(r=>`<details class="bkk-day" id="route-${r.id}" data-route-id="${r.id}"><summary>${esc(r.label)}</summary><p class="eyebrow">${esc(r.area)}</p><p class="route-description">${esc(r.duration)}。${esc(r.condition)}</p><ol>${r.steps.map(s=>`<li><time>${esc(s.time)}</time><div>${s.anchor?`<a href="#${s.anchor}">${esc(s.title)}</a>`:esc(s.title)}<p>${esc(s.text)}</p></div></li>`).join('')}</ol></details>`).join('')}</div>
 ...
 <script type="application/json" id="bkk-planner-data">${JSON.stringify(config).replaceAll('<','\\u003c')}</script><script type="module" src="/trip/bangkok-planner.mjs?v=20260926-1"></script></section>`;
}
