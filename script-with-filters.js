    ...
    <div class="masonry-card ${expired ? 'opacity-60' : ''}">
      ${g.image ? `
        <a href="${g.url}" 
           target="_blank" 
           rel="noopener noreferrer"
           class="masonry-card-image-wrapper block"
           onclick="if(typeof gtag !== 'undefined'){gtag('event', 'click_image', {group_name: '${g.brand.replace(/'/g, "\\'")}', group_category: '${g.category}', event_category: 'engagement', event_label: 'image_click'});}">
          <img src="${g.image}" 
               alt="${g.brand}" 
