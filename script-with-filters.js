...
const CONFIG = {
  ...
  // 🎨 分類設定（可自由調整順序、新增或刪除）
  CATEGORIES: [
    { id: 'food', name: '食品', icon: '🍱', color: 'bg-orange-100 text-orange-700 border-orange-300' },
    { id: 'baby', name: '母嬰', icon: '👶', color: 'bg-pink-100 text-pink-700 border-pink-300' },
    { id: 'fashion', name: '服飾', icon: '👕', color: 'bg-purple-100 text-purple-700 border-purple-300' },
    { id: 'beauty', name: '美妝', icon: '🧴', color: 'bg-rose-100 text-rose-700 border-rose-300' },
    { id: 'home', name: '居家', icon: '🏠', color: 'bg-green-100 text-green-700 border-green-300' },
    { id: '3c', name: '3C', icon: '📱', color: 'bg-blue-100 text-blue-700 border-blue-300' },
    { id: 'travel', name: '旅遊', icon: '✈️', color: 'bg-sky-100 text-sky-700 border-sky-300' },
    { id: 'sport', name: '運動', icon: '🏃', color: 'bg-cyan-100 text-cyan-700 border-cyan-300' },
    { id: 'pet', name: '寵物', icon: '🐾', color: 'bg-amber-100 text-amber-700 border-amber-300' },
    { id: 'other', name: '其他', icon: '🎁', color: 'bg-gray-100 text-gray-700 border-gray-300' }
  ],
  ...
  COUNTRIES: [
    ...
    { id: 'eu', name: '歐洲', flag: '🇪🇺' },
    { id: 'th', name: '泰國', flag: '🇹🇭' },
    { id: 'sea', name: '東南亞', flag: '🌏' },
    { id: 'other', name: '其他', flag: '🌍' }
  ],
...
const utils = {
  ...
  getCategoryConfig: name => {
    ...
    return CONFIG.CATEGORIES.find(c => 
      utils.normalizeFilterValue(c.name) === normalized || 
      c.id === normalized
    );
  },
  
  // 從國家名稱找到對應的配置
  getCountryConfig: name => {
    const normalized = utils.normalizeFilterValue(name);
    return CONFIG.COUNTRIES.find(c => 
      utils.normalizeFilterValue(c.name) === normalized || 
      c.id === normalized
    );
  },
...
function getFilterCounts() {
  ...
  filtered.forEach(g => {
    const cat = utils.normalizeFilterValue(g.itemCategory);
    const country = utils.normalizeFilterValue(g.itemCountry);
    
    if (cat) {
      const config = utils.getCategoryConfig(cat);
      if (config) {
        categoryCounts[config.id] = (categoryCounts[config.id] || 0) + 1;
      }
    }
    
    if (country) {
      const config = utils.getCountryConfig(country);
      if (config) {
        countryCounts[config.id] = (countryCounts[config.id] || 0) + 1;
      }
    }
  });
...
function renderMobileFilters(categoryCounts, countryCounts) {
  // 分類篩選
  const visibleCategories = CONFIG.CATEGORIES.slice(0, CONFIG.MOBILE_FILTER_VISIBLE);
  const hiddenCategories = CONFIG.CATEGORIES.slice(CONFIG.MOBILE_FILTER_VISIBLE);
  ...
  ` + visibleCategories.map(cat => `
    <button onclick="setFilter('category', '${cat.id}')" 
            class="px-3 py-1.5 rounded-lg text-sm font-medium whitespace-nowrap ${
              state.selectedCategory === cat.id ? 'bg-amber-600 text-white' : cat.color
            }">
      ${cat.icon} ${cat.name} ${categoryCounts[cat.id] ? `(${categoryCounts[cat.id]})` : ''}
    </button>
  `).join('');
  ...
  // 國家篩選
  const visibleCountries = CONFIG.COUNTRIES.slice(0, CONFIG.MOBILE_FILTER_VISIBLE);
  const hiddenCountries = CONFIG.COUNTRIES.slice(CONFIG.MOBILE_FILTER_VISIBLE);
  ...
  ` + visibleCountries.map(country => `
    <button onclick="setFilter('country', '${country.id}')" 
            class="px-3 py-1.5 rounded-lg text-sm font-medium whitespace-nowrap ${
              state.selectedCountry === country.id ? 'bg-amber-600 text-white' : 'bg-blue-100 text-blue-700 border-blue-300'
            }">
      ${country.flag} ${country.name} ${countryCounts[country.id] ? `(${countryCounts[country.id]})` : ''}
    </button>
  `).join('');
...
function renderDesktopFilters(categoryCounts, countryCounts) {
  ...
  ` + CONFIG.CATEGORIES.map(cat => `
    <button onclick="setFilter('category', '${cat.id}')" 
            class="w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${
              state.selectedCategory === cat.id ? 'bg-amber-600 text-white font-medium' : `hover:bg-gray-100 ${cat.color.split(' ')[1]}`
            }">
      ${cat.icon} ${cat.name} ${categoryCounts[cat.id] ? `(${categoryCounts[cat.id]})` : ''}
    </button>
  `).join('');
  ...
  ` + CONFIG.COUNTRIES.map(country => `
    <button onclick="setFilter('country', '${country.id}')" 
            class="w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${
              state.selectedCountry === country.id ? 'bg-amber-600 text-white font-medium' : 'hover:bg-gray-100 text-blue-700'
            }">
      ${country.flag} ${country.name} ${countryCounts[country.id] ? `(${countryCounts[country.id]})` : ''}
    </button>
  `).join('');
...
function renderGroupCard(g) {
  ...
  // 取得分類和國家配置
  const categoryConfig = g.itemCategory ? utils.getCategoryConfig(g.itemCategory) : null;
  const countryConfig = g.itemCountry ? utils.getCountryConfig(g.itemCountry) : null;
    ...
    <div class="bg-white rounded-xl overflow-hidden border-2 ${expired ? 'border-gray-300 opacity-60' : 'border-amber-200 hover:border-amber-400 hover:shadow-lg'} transition-all">
      ...
      <div class="p-5">
        ...
        <div class="flex flex-wrap gap-2 mb-3">
          ...
          ${categoryConfig ? `<span class="text-xs ${categoryConfig.color} px-2 py-1 rounded-full border font-medium">${categoryConfig.icon} ${categoryConfig.name}</span>` : ''}
          ${countryConfig ? `<span class="text-xs bg-blue-100 text-blue-700 border-blue-300 px-2 py-1 rounded-full border font-medium">${countryConfig.flag} ${countryConfig.name}</span>` : ''}
        ...
        ${g.endDate && !expired && g.category !== '長期' ? `<div class="mb-3"><button onclick="addToCalendar(event, '${g.brand.replace(/'/g, "\\'")} - 團購截止', '${g.endDate}', '${g.url}', '⏰ 今天是最後一天！記得下單')" class="w-full bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-200 text-blue-700 px-4 py-2 rounded-lg text-sm font-medium hover:from-blue-100 hover:to-indigo-100 transition-colors">📅 加入行事曆（截止提醒）</button></div>` : ''}
...
function renderContent() {
  ...
  const filtered = state.groups.filter(g => {
    ...
    const okCategory = state.selectedCategory === 'all' || 
      (g.itemCategory && utils.normalizeFilterValue(g.itemCategory) === state.selectedCategory) ||
      (g.itemCategory && utils.getCategoryConfig(g.itemCategory)?.id === state.selectedCategory);
    
    // 國家篩選
    const okCountry = state.selectedCountry === 'all' || 
      (g.itemCountry && utils.normalizeFilterValue(g.itemCountry) === state.selectedCountry) ||
      (g.itemCountry && utils.getCountryConfig(g.itemCountry)?.id === state.selectedCountry);
